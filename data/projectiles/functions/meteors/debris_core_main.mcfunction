...
execute if score #tmp despawn_timer matches 0..2000 if score #1S timer matches 0 run particle lava ~ ~ ~ 0.2 0.2 0.2 0 1
execute if score #tmp despawn_timer matches 0..4000 run particle minecraft:white_ash ~ ~ ~ 6 4 6 0 1
execute if score #tmp despawn_timer matches 0..4000 positioned ~-0.5000000001 ~-0.000000001 ~-0.5000000001 as @a[dx=0.000000001,dy=0.00000001,dz=0.000000001,predicate=players:gamemode_sa,predicate=!players:frost_walker,predicate=!players:has_fire_resistance ] unless score @s HurtTime matches 1.. run function projectiles:meteors/touch_debris
...
execute unless score #tmp despawn_timer matches 0..5999 if score #1S timer matches 0 unless entity @a[distance=..32] run fill ~ ~ ~ ~ ~ ~ air replace minecraft:ancient_debris
