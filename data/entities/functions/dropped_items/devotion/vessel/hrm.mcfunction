...
execute as @a if score @s uuid0 = #uuid0 temp if score @s uuid1 = #uuid1 temp if score @s uuid2 = #uuid2 temp if score @s uuid3 = #uuid3 temp run tellraw @a {"text":"You already have an unredeemed vessel!","italic":true,"color":"gray"}