...
execute as @a if score @s uuid0 = #uuid0 temp if score @s uuid1 = #uuid1 temp if score @s uuid2 = #uuid2 temp if score @s uuid3 = #uuid3 temp run function players:items/1kscars/get_player_damage
...
#tellraw @p ["",{"score":{"name":"#pdam","objective":"temp"},"color":"green"},{"text":" | ","color":"gray"},{"score":{"name":"@s","objective":"damage"},"color":"green"}]

execute unless score #pdam temp < @s damage run function players:items/1kscars/hit